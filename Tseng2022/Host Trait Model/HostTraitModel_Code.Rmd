...
pcr_auc_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
pcr_sens_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
pcr_spec_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
comp_auc_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
comp_sens_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
comp_spec_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
<!-- How accurately did infection and competence BRT models distinguish OPV positive and negative species (assuming a threshold value of 0.5)? We evaluate the mean AUC, sensitivity, and specificity of our models, both combined (net) and independently. -->
...
<!-- pm_brts <- readRDS("Output/pm_brts.rds") -->
...
<!-- # Get independent AUC for PCR and Comp models -->
...
<!-- # Get independent sensitivity for PCR and Comp models -->
...
<!-- # Get independent specificity for PCR and Comp models -->
...
rownames(table_perf_metrics) <- c('Host-trait infection model', 'Host-trait competence model', 't-test', 'p-val', "Cohen's d")
...
<!-- # Compare AUC w/ tfun function; extract t-stat & Cohen's d -->
<!-- adata=tfun("testAUC") -->
<!-- adata$tsum$statistic -->
<!-- adata$csum$effsize -->

<!-- # Compare sensitivity w/ tfun function; extract t-stat & Cohen's d -->
<!-- sedata=tfun("sen") -->
<!-- sedata$tsum$statistic -->
<!-- sedata$csum$effsize -->

<!-- # Compare specificity w/ tfun function; extract t-stat & Cohen's d -->
<!-- spdata=tfun("spec") -->
...
ggplot(df_auc)+
  ...
  scale_x_continuous(breaks=c(1,2),
                     labels=levels("RT-PCR", "Virus isolation"),
                     limits=c(0.5,2.5))+
  theme_bw()+
...
# Get predicted probabilities from competence model
...
Tables: *table_predicted_hosts_infection.xlsx*, *table_predicted_hosts_competence.xlsx*
...
# For infection model, create function that sums the number of predicted obs (known and unknown) and just known obs; use sapply to view counts
...
# For competence model, create function that sums the number of predicted obs (known and unknown) and just known obs; use sapply to view counts
...
# For competence model, get sensitivity, specificity, and number of predicted hosts based on various threshold values
...
### INTERMISSION: Manuscript Table 1 ###
# What is the sensitivity and specificity of the models when we fix threshold to 0.5?
#Infection/PCR
InformationValue::sensitivity(pred$PCR, pred$pred_pcr, threshold = 0.5)
InformationValue::specificity(pred$PCR, pred$pred_pcr, threshold = 0.5)
#Competence/virus isolation
InformationValue::sensitivity(pred$competence, pred$pred_comp, threshold = 0.5)
InformationValue::specificity(pred$competence, pred$pred_comp, threshold = 0.5)
...
bdata <- bdata %>% dplyr:::mutate(pred_mean = rowMeans(across(pred_pcr:pred_comp), na.rm = TRUE))
...
#Save
# png("Output/tree_pp_clades2_gray.png",width=12,height=6,units="in",res=300)
