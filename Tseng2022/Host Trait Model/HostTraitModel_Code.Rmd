...
The following code reproduces the analyses from <...>, pertaining to the host-trait model - the model trained on host traits alone based on two evidence types: PCR positivity vs. virus isolation. Please note that these models in the code are referred to as the infection vs. competence models, but in the manuscript, are referred to as the exposure vs. susceptible model, respectively.  The code is subdivided into seven parts (see Table of Contents below). To reproduce the analyses pertaining to the link prediction model, please see the markdown file *LinkPrediction_Code.Rmd* located in the [PoxHost repository][1].
...
library(pROC) #get auc
library(InformationValue) #get sens/spec
library(tidyr)
library(ggplot2)
# library(sciplot)
library(fastDummies)
...
### *Evaluate performance measures - ALT *
How accurately did infection and competence BRT models distinguish OPV positive and negative species (assuming a threshold value of 0.5)? We evaluate the mean AUC, sensitivity, and specificity of each of our independent models
...
pm_brts <- readRDS("Output/pm_brts.rds") #100 elements

# Did traits predict citation counts? 
...
pcr_auc_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))
...
pcr_sens_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))
...
pcr_spec_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))
...
comp_auc_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))
...
comp_sens_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))

# Get specificity for each iteration and calculate mean sensitivity and standard error across iterations
...
comp_spec_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", formatC(se, format="e"))

# Clean environment; save performance metrics to create a table in the next section
rm(comp_cpreds, pcr_cpreds, comp_keep, keep, pcr_keep, mean, se)

```

### *Compare performance between BRTs trained on PCR vs. virus isolation data*
Is model performance significantly different when training on PCR vs. virus isolation data? We create a function to perform an unpaired T-test on the AUC, sensitivity and specificity of the two models and determine the effect size (mean difference) via Cohen's d.

```{r perf_compare}

### T-test for AUC ###

# Get auc values for PCR model
adata_pcr=data.frame(y=c(sapply(pcr_auc, function(x) x)))
adata_pcr$seed = 1:nrow(adata_pcr)
adata_pcr$response = "infection"

# Get auc values for virus isolatino model
...
adata_auc$response=factor(adata_auc$response,levels=c('infection','competence'))
...
# Transform 'response' to factor
adata_sens$response=factor(adata_sens$response,levels=c('infection','competence'))
...
# Transform 'response' to factor
adata_spec$response=factor(adata_spec$response,levels=c('infection','competence'))
...
table_perf_metrics <- data.frame(matrix(c(pcr_auc_mean, pcr_sens_mean, pcr_spec_mean, 
                                          comp_auc_mean, comp_sens_mean, comp_spec_mean, 
...
rownames(table_perf_metrics) <- c('Host exposure model', 'Susceptible host model', 't-test', 'p-val', "Cohen's d")
write.csv(table_perf_metrics, "Output/table_perf_metrics.csv")

rm(list=setdiff(ls(), c("pcr_brts", "comp_brts", "pm_brts", 
...
df_auc$response2=plyr::revalue(df_auc$response, c("infection"="RT-PCR positivity","competence"="Virus isolation"))
sdata$response2=plyr::revalue(sdata$response, c("infection"="RT-PCR positivity","competence"="Virus isolation"))
...
ggplot(df_auc)+
  ...
  scale_x_continuous(breaks=c(1,2),
                     labels=levels(df_auc$response2),
                     limits=c(0.5,2.5))+
  theme_bw()+
...
pcr_brts <- readRDS("Output/pcr_brts.rds")
comp_brts <- readRDS("Output/comp_brts.rds")

# Keep only first 100 iterations
pcr_brts=pcr_brts[1:100]
comp_brts=comp_brts[1:100]
...
Tables: *table_thresholdmoving_pcr.csv*, *table_thresholdmoving_comp.csv*, *table_predicted_hosts_infection.xlsx*, *table_predicted_hosts_competence.xlsx*
...
write.csv(table_ts_pcr, file='Output/table_thresholdmoving_pcr.csv')
write.csv(table_ts_comp, file='Output/table_thresholdmoving_comp.csv')
...
p1=gg+
  ...
  scale_colour_manual(values=c("#FDE725FF","gray60"), name="Host positivity")
...
pp_tree2=ggarrange(p1,p2,
              labels=c("(A) PCR ReqSens0.8: Th=0.38","(B) PCR MaxSensSpec: Th=0.36"),
              # font.label(size=10),
              # hjust=-0.6,
              # label.x=c(0.1,0.1),
              # label.y=c(1,1),
...
#Save
png("Output/tree_pp_clades2_red.png",width=12,height=6,units="in",res=600)
pp_tree2
dev.off()
...
# What was the relative importance/influence of each feature in the PCR model? Get values.
vinf=lapply(pcr_brts,function(x) x$rinf)
pcr_vinf=do.call(rbind,vinf)

# What was the relative influence of each feature in the virus isolation model? Get values.
vinf=lapply(comp_brts,function(x) x$rinf)
comp_vinf=do.call(rbind,vinf)

# What was the mean, standard error, and variance of feature relative influence in the PCR model?
...
# What was the mean, standard error, and variance of feature relative influence in the virus isolation model?
...
# How does the average variance compare between the PCR and virus isolation model?
mean(vdata_pcr$rvar)
mean(vdata_comp$rvar)

# How does the variance of relative influence compare between the PCR and virus isolation model?
...
# Get the relative importance values for the top 10 features of the PCR model
vinf=lapply(pcr_brts,function(x) x$rinf)
pcr_vinf=do.call(rbind,vinf)
...
pcr_vinf$var <- ifelse(pcr_vinf$var=="adult_mass_g", "Adult mass (g)", pcr_vinf$var)
...
pcr_vinf$type <- ifelse(pcr_vinf$var=="Adult mass (g)", "Life history", pcr_vinf$type)
pcr_vinf$type <- ifelse(pcr_vinf$var=="Female maturity (days)", "Life history", pcr_vinf$type)
  
# Boxplot relative feature importance for the PCR model
library(viridis)
library(stringr)
boxplot_trait_pcr <- ggplot(pcr_vinf) + ggtitle("(A) Host exposure model") +
        ...
        scale_fill_manual(values=c("#482677FF","#55C667FF","#238A8DFF")) +
        scale_y_discrete(labels = function(x) str_wrap(x, width = 16))

# Save boxplots of relative feature importance
png("Output/boxplot_trait_ranking_pcr.png",width=6.12,height=10,units="in",res=600)
boxplot_trait_pcr
dev.off()

# Get the relative importance values for the top 10 features of the virus isolation model
vinf=lapply(comp_brts,function(x) x$rinf)
comp_vinf=do.call(rbind,vinf)
...
comp_vinf$var <- ifelse(comp_vinf$var=="max_longevity_d", "Maximum longevity (days)", comp_vinf$var)
comp_vinf$var <- ifelse(comp_vinf$var=="interbirth_interval_d", "Interbirth interval (days)", comp_vinf$var)
comp_vinf$var <- ifelse(comp_vinf$var=="litters_per_year_n", "Litters per year", comp_vinf$var)
comp_vinf$var <- ifelse(comp_vinf$var=="female_maturity_d", "Female maturity (days)", comp_vinf$var)
comp_vinf$var <- ifelse(comp_vinf$var=="adult_mass_g", "Adult mass (g)", comp_vinf$var)
...
comp_vinf$type <- ifelse(comp_vinf$var=="Maximum longevity (days)", "Life history", comp_vinf$type)
comp_vinf$type <- ifelse(comp_vinf$var=="Interbirth interval (days)", "Life history", comp_vinf$type)
comp_vinf$type <- ifelse(comp_vinf$var=="Litters per year", "Life history", comp_vinf$type)
comp_vinf$type <- ifelse(comp_vinf$var=="Female maturity (days)", "Life history", comp_vinf$type)
comp_vinf$type <- ifelse(comp_vinf$var=="Adult mass (g)", "Life history", comp_vinf$type)
...
png("Output/boxplot_trait_ranking_comp.png",width=5.7,height=10,units="in",res=600)
boxplot_trait_comp
dev.off()
...
# Were any features consistenly high or low ranking across both PCR and virus isolation models? Flag.
n=7
