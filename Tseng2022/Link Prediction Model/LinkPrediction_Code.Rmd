...
(2) PresenceAbsence_OPV.xlsx, the excel file of OPV genome annotations
...
#(4) View frequency of various virus species
table <- data.frame(table(genes$VirusSpecies))
table$Perc <- table$Freq/sum(table$Freq)*100

#(5) Drop accessory genes that are present in only one virus species (all 0's except for one)
genes <- genes[c(1:4,4 + which(colSums(genes[-(1:4)])>1))]

#(6) Identify observations of the same host-virus links with identical presence/absence of accessory genes 
genes$dup <- duplicated(genes[,-c(1:2)])
table(genes$dup)
#45 dups

#(7) Drop duplicate observations
genes <- genes[genes$dup==FALSE,]
genes$dup=NULL

#(8) View frequency of various virus species
table <- data.frame(table(genes$VirusSpecies))
table$Perc <- table$Freq/sum(table$Freq)*100
...
genes_mat <- subset(genes,select=-c(Accession,VirusSpecies,HostGenus))
...
#(7) Screeplot of Variance or Eigenvalues: shows the decreasing rate at which variance is explained by additional PCs
...
genes_cat <- subset(genes,select=-c(Accession,VirusSpecies,HostGenus))
