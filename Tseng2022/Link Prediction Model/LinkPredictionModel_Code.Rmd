...
The following code reproduces the analyses from *Viral genomic features predict orthopoxvirus reservoir hosts*, pertaining to the link prediction model. The code is subdivided into seven parts (see Table of Contents below). To reproduce the analyses pertaining to the host prediction model, please see the markdown file *HostTraitModel_Code.Rmd* located in the [PoxHost repository][1].

To run the following script, four files are required in your working directory, in addition to an 'Output' folder:
...
Figures: *screeplot_dim_pca.png*, *screeplot_var_pca.png*, *plot_cumvar_pca.png*, *plot_pca_scoresbyspecies.pdf*, *plot_loadins_pca_dim1and2.png*, *plot_loadings_pca_dim3and4.png*, *biplot_pca_dim1and2.png*,
...
# Style 3: Plot sequences by virus species w/ viridis
  ...
  library(cowplot)
  pdf("Output/plot_pca_scoresbyspecies.pdf", width=8,height=8)
...
# Style 1: Plot gene loadings only
  ...
  ## Dim 1 and 2
  png("Output/plot_loadings_pca_dim1and2.png", width=6,height=4,units="in",res=600)
  fviz_pca_var(pca, 
...
### Merge dfs of highly contributing gene loadings w/ functional roles ###

# Read in data
 
  # PC1 positive loadings: read in data, rename columns, and merge
  pc1_fnx_pos <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC1")[,1:3]

  # PC1 negative loadings: read in data, rename columns, and merge
  pc1_fnx_neg <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC1")[,c(7,9,10)]

  # PC3 positive loadings: read in data, rename columns, and merge
  pc3_fnx_pos <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC3")[,1:3]

  # PC3 negative loadings: read in data, rename columns, and merge
  pc3_fnx_neg <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC3")[,7:8]
  pc3_fnx_neg$role <- NA

  # PC4 positive loadings: read in data, rename columns, and merge
  pc4_fnx_pos <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC4")[,1:3]

  # PC4 negative loadings: read in data, rename columns, and merge
  pc4_fnx_neg <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC4")[,8:10]

  # PC9 positive loadings: read in data, rename columns, and merge
  pc9_fnx_pos <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC9")[,1:3]

  # PC1 negative loadings: read in data, rename columns, and merge
  pc9_fnx_neg <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC9")[,7:9]

# Clean dataframes of loadings and combine into single dataframe
  ...
  # For each object in the list, get the names of each unique functional role to identify mistakes in spelling 
  list_names <- list()
  for(i in seq_along(list_df)) {
  ...
  # Correct naming of functional roles
  list_names <- list()
  for(i in seq_along(list_df)) {
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Viron/Membrane", "Virion (in an infectious virus particle)", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Kelch-Like Protein", "Kelch-like protein", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="DNA Replication", "DNA replication", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Cell death/ cell cycle regulation", "Cell death/cell cycle regulation", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="RNA/Transcription/Translation", "RNA/transcription/translation", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Addaptive immunity/Anitibody", "Adaptive immunity/antibody", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Cytokine/chemokine regulation", "Chemokine/cytokine regulation", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Virus Entry", "Virus entry", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Ckemokine/cytokine regulation", "Chemokine/cytokine regulation", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Virion/Membrane", "Virion (in an infectious virus particle)", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Addaptive Immunity", "Adaptive immunity/antibody", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Adaptive immunity/ Antibody", "Adaptive immunity/antibody", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Adaptive immunity/Anitibody", "Adaptive immunity/antibody", list_df[[i]]$role)
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Addaptive immunity/ Antibody", "Adaptive immunity/antibody", list_df[[i]]$role)  
      list_df[[i]]$role <- ifelse(list_df[[i]]$role =="Kelch-like Protein", "Kelch-like protein", list_df[[i]]$role)
      list_names[[i]] <- unique(list_df[[i]]$role)
  }
...
# Unlist and merge all rows of data
...
write.csv(gene_roles_dup, "Output/gene_roles_dup.csv")

# For now, we drop duplicates and try plotting loadings by functional role
gene_roles_temp <- gene_roles[!duplicated(gene_roles$gene),]
fviz_pca_var(pca, axes = c(1, 2),
               # col.var = "contrib", #change to functional role
               fill.var = gene_roles_temp$role,
               # gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
               # palette = cbPalette,
               repel = TRUE,
               labelsize=1,
               select.var=list(name=pc1_sd))   #character vector of vars to be drawn
  
### For each PC, extract the genes that are within the top 1.5*SD and the bottom 1.5*SD

  # PC1 loadings
  pc1 <- rank_PC[,1:2]
  colnames(pc1) <- c("gene", "loading")
  sd <- sd(pc1$loading)
  pc1_pos <- pc1 %>% filter(loading >= (max(pc1$loading)-1.5*sd))
  pc1_neg <- pc1 %>% filter(loading <= (min(pc1$loading)+1.5*sd))
  pc1_gene <- c(pc1_pos$gene, pc1_neg$gene)
  # pc1_pos_alt <- pc1 %>% filter(loading > (mean(pc1$loading)+1.5*sd))
  # pc1_neg_alt <- pc1 %>% filter(loading < (mean(pc1$loading)-1.5*sd))

  # PC2 loadings
  pc2 <- rank_PC[,3:4]
  colnames(pc2) <- c("gene", "loading")
  sd <- sd(pc2$loading)
  pc2_pos <- pc2 %>% filter(loading >= (max(pc2$loading)-1.5*sd))
  pc2_neg <- pc2 %>% filter(loading <= (min(pc2$loading)+1.5*sd))
  ...
  sd <- sd(pc3$loading)
  pc3_pos <- pc3 %>% filter(loading >= (max(pc3$loading)-1.5*sd))
  pc3_neg <- pc3 %>% filter(loading <= (min(pc3$loading)+1.5*sd))
  ...
  sd <- sd(pc4$loading)
  pc4_pos <- pc4 %>% filter(loading >= (max(pc4$loading)-1.5*sd))
  pc4_neg <- pc4 %>% filter(loading <= (min(pc4$loading)+1.5*sd))
  ...
  sd <- sd(pc5$loading)
  pc5_pos <- pc5 %>% filter(loading >= (max(pc5$loading)-1.5*sd))
  pc5_neg <- pc5 %>% filter(loading <= (min(pc5$loading)+1.5*sd))
  ...
  sd <- sd(pc6$loading)
  pc6_pos <- pc6 %>% filter(loading >= (max(pc6$loading)-1.5*sd))
  pc6_neg <- pc6 %>% filter(loading <= (min(pc6$loading)+1.5*sd))
  ...
  sd <- sd(pc7$loading)
  pc7_pos <- pc7 %>% filter(loading >= (max(pc7$loading)-1.5*sd))
  pc7_neg <- pc7 %>% filter(loading <= (min(pc7$loading)+1.5*sd))
  ...
  sd <- sd(pc8$loading)
  pc8_pos <- pc8 %>% filter(loading >= (max(pc8$loading)-1.5*sd))
  pc8_neg <- pc8 %>% filter(loading <= (min(pc8$loading)+1.5*sd))
  ...
  sd <- sd(pc9$loading)
  pc9_pos <- pc9 %>% filter(loading >= (max(pc9$loading)-1.5*sd))
  pc9_neg <- pc9 %>% filter(loading <= (min(pc9$loading)+1.5*sd))
  ...
  sd <- sd(pc10$loading)
  pc10_pos <- pc10 %>% filter(loading >= (max(pc10$loading)-1.5*sd))
  pc10_neg <- pc10 %>% filter(loading <= (min(pc10$loading)+1.5*sd))
  pc10_gene <- c(pc10_pos$gene, pc10_neg$gene)

### Create loading plots including only the most influential genes for each PC
  ...
  library(cowplot)
  pdf("Output/plot_pca_loadings_nolabels.pdf", width=8,height=8)
  plot_grid(plot_pc12, plot_pc34, plot_pc56, plot_pc78, plot_pc910, legend, ncol=3, nrow=2, scale=c(0.8), labels = c('A','B','C','D','E',''))  
  dev.off()

### Create biplots including only the most influential genes for each PC
  ...
  # PC1 and PC2
  pc12_gene <- unique(c(pc1_gene, pc2_gene))
  biplot_pc12 <- fviz_pca_biplot(pca, axes=c(1,2), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
                  gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                  label="none",
                  select.var=list(name=pc12_gene)) +
  ...
  # PC3 and PC4
  pc34_gene <- unique(c(pc3_gene, pc4_gene))
  biplot_pc34 <- fviz_pca_biplot(pca, axes=c(3,4), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
  ...
  # PC5 and PC6
  pc56_gene <- unique(c(pc5_gene, pc6_gene))
  biplot_pc56 <- fviz_pca_biplot(pca, axes=c(5,6), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
  ...
  # PC7 and PC8
  pc78_gene <- unique(c(pc7_gene, pc8_gene))
  biplot_pc78 <- fviz_pca_biplot(pca, axes=c(7,8), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
  ...
  # PC9 and PC10
  pc910_gene <- unique(c(pc9_gene, pc10_gene))
  biplot_pc910 <- fviz_pca_biplot(pca, axes=c(9,10), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
  ...
  ## Get legend only
  legend <- cowplot::get_legend(fviz_pca_biplot(pca, axes=c(9,10), geom.ind="point", 
                                 pointshape=21, pointsize=1, 
                  fill.ind=genes$VirusSpecies, col.ind="black", addEllipses=TRUE, 
                  palette=cbPalette, alpha.ind=0.5, 
                  col.var = "contrib", 
  ...
  ## Combine biplots and save
  library(cowplot)
  pdf("Output/biplot_pca_nature.pdf", width=8,height=8)
  plot_grid(biplot_pc12, biplot_pc34, biplot_pc56, biplot_pc78, biplot_pc910, legend, ncol=3, nrow=2, scale=c(0.8), labels = c('A','B','C','D','E',''))  
  dev.off()
  
### For each PC, extract the genes that are greater than 1.5*sd and less than -1.5*sd
...
#   # Combine loadings of top contributing genes
#   list_df <- ls()[grepl("_sd", ls())]
#   top_pcgenes <- rbind(pc1_pos_sd,pc1_neg_sd,pc3_pos_sd,pc3_neg_sd,pc4_pos_sd,pc4_neg_sd,pc9_pos_sd,pc9_neg_sd)
# 
#   # Clean environment
#   rm(list = ls()[grepl("pc1|pc3|pc4|pc9", ls())])
...
load("/Users/katietseng/Desktop/HPC/HPC_30Jan2024/Kamiak_Results_Model1/brts_Model1.RData")
brts1=brts
rm(brts)
...
### *Evaluate performance measures for Model 1 (all links), Model 2 (excluding Vaccinia links), and Model 3 (all links; host traits only)*
How accurately did the different models distinguish host-virus links from non-links (assuming a threshold value of 0.5)? We evaluate the mean AUC, sensitivity, and specificity of our models.

```{r perf_auc}

### LOAD DATA ###

# Get data from Model 1 - includes VACV links
load("/Users/katietseng/Desktop/HPC/HPC_30Jan2024/Kamiak_Results_Model1/brts_Model1.RData")
brts1=brts
# pm_brts1=pm_brts
rm(brts, pm_brts)

# Get data from Model 2 - excludes VACV links
load("/Users/katietseng/Desktop/HPC/HPC_30Jan2024/Kamiak_Results_Model2/brts_Model2.RData")
brts2=brts
# pm_brts2=pm_brts
rm(brts, pm_brts)

# Get data from Model 3 - includes host traits only (and VACV links); excludes viral/PC variables
load("/Users/katietseng/Desktop/HPC/HPC_30Jan2024/Kamiak_Results_Model3/brts_Model3.RData")
brts3=brts
# pm_brts3=pm_brts
rm(brts, pm_brts)
...
### MODEL 1 ### includes VACV links; get model performance measures assuming 0.5 threshold
...
### MODEL 2 ### excludes VACV links; get model performance measures assuming 0.5 threshold
...
### MODEL 3 ### includes host traits only; get model performance measures assuming 0.5 threshold
...
# Get AUC for each iteration and calculate mean AUC and standard error across iterations
m3_auc = lapply(m3_cpreds, function(x) auc(x$link, x$cpred))
mean = mean(sapply(m3_auc,function(x) x))
se = std.error(sapply(m3_auc,function(x) x))
m3_auc_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
m3_sens_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))
...
m3_spec_mean <- paste0(sprintf("%.2f",mean), " \u00b1 ", sprintf("%.3f",se))

# Clean environment; save performance metrics to create a table in the next section
rm(m1_cpreds, m2_cpreds, m3_cpreds, keep1, keep2, keep3, mean, se)

```

### *Compare performance between BRTs trained on host and viral traits (Models 1 and 2) as opposed to host traits alone (Model 3)*
Is model performance significantly different when trained on all links (Model 1) vs. links excluding VACV virus (Model 2)? Is model performance significantly different when trained on host AND viral traits (Model 1) as opposed to host traits alone (Model 3)? We create a function to perform an unpaired T-test on the AUC, sensitivity and specificity of the models being compared and determine the effect size (mean difference) via Cohen's d.
...
# get auc values for model 3
adata_m3=data.frame(y=c(sapply(m3_auc, function(x) x)))
adata_m3$seed = 1:nrow(adata_m3)
adata_m3$response = "model3"

# combine auc values for models 1-3: will need this for later {r perf_boxplot}
adata_auc123 = rbind(adata_m1, adata_m2, adata_m3)

# combine auc values for models 1 and 2
...
# t-test
tsum_auc12=t.test(y~response,data=adata_auc,
...
# combine auc values for models 1 and 3
...
# t-test
tsum_auc13=t.test(y~response,data=adata_auc,
            alternative='two.sided',
            var.equal=F,paired=F)  
...
# get sensitivity values for model 3
adata_m3=data.frame(y=c(sapply(m3_sens, function(x) x)))
adata_m3$seed = 1:nrow(adata_m3)
adata_m3$response = "model3"

# combine sensitivity values for models 1-3: will need this for later {r perf_boxplot}
adata_sens123 = rbind(adata_m1, adata_m2, adata_m3)

# combine sensitivity values for models 1 and 2
...
# t-test
tsum_sens12=t.test(y~response,data=adata_sens,
            alternative='two.sided',
            var.equal=F,paired=F)  
...
# combine sensitivity values for models 1 and 3
...
# t-test
tsum_sens13=t.test(y~response,data=adata_sens,
            alternative='two.sided',
            var.equal=F,paired=F)  
...
# get specificity values for model 1 
adata_m1=data.frame(y=c(sapply(m1_spec, function(x) x)))
adata_m1$seed = 1:nrow(adata_m1)
adata_m1$response = "model1"

# get specificity values for model 2
adata_m2=data.frame(y=c(sapply(m2_spec, function(x) x)))
adata_m2$seed = 1:nrow(adata_m2)
adata_m2$response = "model2"

# get specificity values for model 3
adata_m3=data.frame(y=c(sapply(m3_spec, function(x) x)))
adata_m3$seed = 1:nrow(adata_m3)
adata_m3$response = "model3"

# combine specificity values for models 1-3: will need this {r perf_boxplot}
adata_spec123 = rbind(adata_m1, adata_m2, adata_m3)

# combine specificity values for models 1 and 2
...
# t-test
tsum_spec12=t.test(y~response,data=adata_spec,
            alternative='two.sided',
            var.equal=F,paired=F)  
...
# combine specificity values for models 1 and 3
...
# t-test
tsum_spec13=t.test(y~response,data=adata_spec,
            alternative='two.sided',
            var.equal=F,paired=F)  
...
ps=c(tsum_auc12$p.value,
     tsum_sens12$p.value,
     tsum_spec12$p.value)
ps_adjust12 <- sprintf("%.4f",p.adjust(ps,method="BH"))    #"BH" (aka "fdr") = Benjamini & Hochberg (1995) method control the false discovery rate, the expected proportion of false discoveries amongst the rejected hypotheses (false discovery rate is less stringent condition than family-wise error rate)
...
table_perf_metrics <- data.frame(matrix(c(m1_auc_mean, m1_sens_mean, m1_spec_mean, 
                                          ...
                                          sprintf("%.2f", tsum_auc12$statistic), 
                                          sprintf("%.2f", tsum_sens12$statistic), 
                                          sprintf("%.2f", tsum_spec12$statistic),
                                          ps_adjust12, 
                                          sprintf("%.2f", csum_auc12$effsize), 
                                          sprintf("%.2f", csum_sens12$effsize), 
                                          sprintf("%.2f", csum_spec12$effsize), 
                                          
                                          m3_auc_mean, m3_sens_mean, m3_spec_mean,
                                          sprintf("%.2f", tsum_auc13$statistic), 
                                          sprintf("%.2f", tsum_sens13$statistic), 
                                          sprintf("%.2f", tsum_spec13$statistic),
                                          ps_adjust13,
                                          sprintf("%.2f", csum_auc13$effsize), 
                                          sprintf("%.2f", csum_sens13$effsize), 
                                          sprintf("%.2f", csum_spec13$effsize)), 
                                  ncol=3, byrow=TRUE))

# rename columns and rownames
colnames(table_perf_metrics) <- c('AUC (mean \u00b1 SE)', 'Sensitivity (mean \u00b1 SE)', 'Specificity (mean \u00b1 SE)')
rownames(table_perf_metrics) <- c('Link prediction model', 'Link prediction model excluding VACV associations', 't-test (Model 1 vs. 2)', 'p-val (Model 1 vs. 2)', "Cohen's d (Model 1 vs. 2)", 'Link prediction model with host traits only', 't-test (Model 1 vs. 3)', 'p-val (Model 1 vs. 3)', "Cohen's d (Model 1 vs. 3)")

# save table
write.csv(table_perf_metrics, "Output/table_perf_metrics.csv")

# clean environment
rm(list=setdiff(ls(), c("brts1", "brts2", "brts3",
                        "adata_auc123", "adata_sens123", "adata_spec123")))
...
# row-bind predicted values of all iterations for Model 3
...
png("Output/ROCcurve_thv0.5_model2.png",width=8,height=6,units="in",res=300)
...
png("Output/ROCcurve_thv0.5_model3.png",width=8,height=6,units="in",res=300)
...
# get optimum THV by finding THV when sens+spec are maximized for Model 3
...
### OPTION 3 ### Get optimum THV based on the ROC curve: maximize TRP and FPR (true/false positive rate) by minimizing |FPR + TPR - 1|, which is equal to minimizing |sensitivity - specificity|.
...
# transform 'response' to factor
adata_auc123$response=factor(adata_spec123$response,levels=c('model1', 'model2', 'model3'))
adata_sens123$response=factor(adata_spec123$response,levels=c('model1', 'model2', 'model3'))
adata_spec123$response=factor(adata_spec123$response,levels=c('model1', 'model2', 'model3'))

# create list of dataframes
list_dfs <- list(adata_auc123, adata_sens123, adata_spec123)
...
ggplot(df_auc)+
  ...
  scale_x_continuous(breaks=c(1,2),
                     ...
                     limits=c(0.5,2.5))+
  coord_cartesian(ylim = c(0.92, 0.97)) +
  theme_bw()+
...
### BOXPLOTS FOR ALL MODELS (Nature) ###

# AUC
png("Output/boxplot_model_auc_nature.png",width=4,height=5,units="in",res=300)
set.seed(1)
ggplot(df_auc)+
  geom_boxplot(aes(x=x,y=y,group=x),width=0.5,alpha=0.25,outlier.alpha=0)+
  geom_point(aes(x=xj,y=y),size=1.5,alpha=0.5)+
  scale_x_continuous(breaks=c(1,2,3),
                     labels=c("Including\n VACV links","Excluding\n VACV links","Trained on\n host traits only"),
                     limits=c(0.5,3.5))+
  coord_cartesian(ylim = c(0.65, 0.97)) +
  theme_bw()+
  labs(x="Link prediction models",
       y="Area under the curve (AUC)")+
  theme(axis.text=element_text(size=10),
        axis.text.x=element_text(size=8),
        axis.title=element_text(size=12))+
...
# sensitivity and specificity
png("Output/boxplot_model_senspec_nature.png",width=4,height=5,units="in",res=300)
set.seed(1)
ggplot(sdata) +
  geom_boxplot(aes(x=x,y=y,group=x),width=0.5,alpha=0.25,outlier.alpha=0)+
  geom_point(aes(x=xj,y=y),size=1.5,alpha=0.5)+
...
# Get predicted probabilities from Models 1-3
...
# Combine predictions from Models 1-3
...
apreds$type2=revalue(apreds$type,c("Model1"="FullModel","Model2"="PartialModel","Model3"="HostTraitModel"))
...
apreds$type2=revalue(apreds$type2,
                     c("FullModel"="Full Model",
                       ...
                       "HostTraitModel"="Host Trait Model"))
...
write.csv(preds3,"Output/table_LinkPredictionModel3_predictions.csv")
...
# Load file
pred3 <- read.csv("Output/table_LinkPredictionModel3_predictions.csv")
...
# save threshold values 
write.csv(ts3, file='Output/table_thresholdvalues_model3.csv')
...
pred_model3 <- list('known'=pred_kn, 
                    ...
                    'unknown_rs0.9'=pred_unk_rs0.9, 
                    'unknown_mss3'=pred_unk_mss3)
write.xlsx(pred_model3, file='Output/table_LinkPredictionModel3_PredictedLinks.xlsx')
...
virus_ratio_model3 <- list('rs0.8' = virus_ratio_rs0.8,
                           ...
                           'rs0.9' = virus_ratio_rs0.9, 
                           'mss0.3' = virus_ratio_mss0.3)
write.xlsx(virus_ratio_model3, file = 'Output/table_LinkPredictionModel3_virusratio.xlsx')
...
cor(apreds2$pred_Model1,apreds2$pred_Model3,method='spearman') #computes Spearman correlation coefficient
cor.test(apreds2$pred_Model1,apreds2$pred_Model3,method='spearman') #tests for correlation b/w paired samples 
...
Model3_lmod=pgls(pred_Model3~1,data=cdata,lambda="ML")
...
map_host_rs0.9_supplementary <- rasterVis::levelplot(maps_hostlink,  
                     col.regions = mycolors,
                     alpha = 0.5, 
                     scales=list(alternating=FALSE),
                     par.strip.text=list(cex=0),
...
png("Output/Nature/map_host_rs0.9_supplementary.png",width=6,height=8,units="in",res=600)
map_host_rs0.9_supplementary
dev.off()

pdf("Output/Nature/map_host_rs0.9_supplementary.pdf",width=6,height=8)
map_host_rs0.9_supplementary
dev.off()


#############################################################
#### Main Text: Map of link prediction model predictions ####
#############################################################
...
map_link_rs0.9_manuscript <- rasterVis::levelplot(maps_link,  
                     col.regions = mycolors,
                     alpha = 0.5, 
                     scales=list(alternating=FALSE),
                     par.strip.text=list(cex=0),
...
png("Output/map_link_rs0.9_manuscript.png",width=6,height=8,units="in",res=600)
map_link_rs0.9_manuscript
dev.off()

pdf("Output/map_link_rs0.9_manuscript.pdf",width=6,height=8)
map_link_rs0.9_manuscript
dev.off()
...
map_susceptible_manuscript <- rasterVis::levelplot(map_susceptible,  
                     col.regions = mycolors,
...
png("Output/map_susceptible_manuscript.png",width=6,height=4,units="in",res=600)
map_susceptible_manuscript
dev.off()

pdf("Output/map_susceptible_manuscript.pdf",width=6,height=4)
map_susceptible_manuscript
dev.off()
...
##### Human Susceptibility: https://github.com/bansallab/mpx_landscape #####
...
# Combine link prediction and susceptiblity maps
...
maps_nature <- rasterVis::levelplot(maps,  
                     col.regions = mycolors,
                     alpha = 0.5, 
                     scales=list(alternating=FALSE), #prevents axis labels from alternating from one side of group of panels to the other
                     par.strip.text=list(cex=0),
...
png("Output/Nature/maps_nature.png",width=8,height=12,units="in",res=600)
maps_nature
dev.off()


#############################################################
#### Main Text: Map of Mpox link prediction model predictions with susceptibility ####
#############################################################
...
#(7) Pull out the relevant lists of known hosts and predicted hosts
pred1 %>% filter(link1==1) %>% dplyr::pull(treename) %>% gsub("_"," ",.) -> known.1 #n=19
pred1 %>% filter(bin_rs0.9==1) %>% dplyr::pull(treename) %>% gsub("_"," ",.) -> pred.1_rs0.9 #n=19
...
png("Output/Nature/maps_nature_mpxv.png",width=8,height=12,units="in",res=600)
maps_nature
dev.off()
...
8. Nature Micro: Model Performance Figures
===========================
...
### *Get performance measures for link prediction models*

```{r perf_link}

# Set working directory for reading in link prediction model results
setwd("~/Library/CloudStorage/OneDrive-WashingtonStateUniversity(email.wsu.edu)/Fernandez Lab/Projects (Active)/OPV Host Prediction/GitHub/PoxHost/Tseng2022/Link Prediction Model")

# Get data from Link Prediction Model 1
load("~/Fernandez Lab Dropbox/Katie Tseng/Mac/Downloads/HPC_24Nov2022_Model1/Output/brts_Model1.RData")
brts1=brts
rm(brts)

# Get data from Link Prediction Model 2
load("~/Fernandez Lab Dropbox/Katie Tseng/Mac/Downloads/HPC_24Nov2022_Model2/Output/brts_Model2.RData")
brts2=brts
rm(brts)
...
# Get AUC, sens, and spec for each iteration and calculate mean AUC and standard error across iterations
m1_auc = lapply(m1_cpreds, function(x) auc(x$link, x$cpred))
m1_sens = lapply(m1_cpreds, function(x) InformationValue::sensitivity(x$link, x$cpred, threshold=0.5))
m1_spec = lapply(m1_cpreds, function(x) InformationValue::specificity(x$link, x$cpred, threshold=0.5))
...
adata_m2$seed = 1:nrow(adata_m2)
adata_m2$response = "model2"
adata_auc_link=rbind(adata_m1, adata_m2)

# Transform 'response' to factor
adata_auc_link$response=factor(adata_auc_link$response,levels=c('model1','model2'))
...
adata_m2$seed = 1:nrow(adata_m2)
adata_m2$response = "model2"
adata_sens_link=rbind(adata_m1, adata_m2)

# Transform 'response' to factor
adata_sens_link$response=factor(adata_sens_link$response,levels=c('model1','model2'))
...
adata_m2$seed = 1:nrow(adata_m2)
adata_m2$response = "model2"
adata_spec_link=rbind(adata_m1, adata_m2)

# Transform 'response' to factor
adata_spec_link$response=factor(adata_spec_link$response,levels=c('model1','model2'))
 
# Clean environment
rm(list=setdiff(ls(), c("brts1", "brts2", 
                        "adata_auc_link", "adata_sens_link", "adata_spec_link")))

```

### *Get performance measures for host-trait models*

```{r perf_host}

# Set working directory for reading in host-trait model results
...
# Load data
pcr_brts <- readRDS("Output/pcr_brts.rds") #101 elements
comp_brts <- readRDS("Output/comp_brts.rds") #101 elements

# Index and save non-missing BRT iterations
...
# Check number of iterations (only keep 100)
length(keep)
keep=keep[!keep==101]

# Trim, keeping only 100 iterations that are non-missing 
pcr_brts=pcr_brts[keep]
comp_brts=comp_brts[keep]

### INFECTION MODEL: Get model performance measures assuming 0.5 threshold ###

# Get predicted probabilities from infection model for each iteration
pcr_cpreds=lapply(pcr_brts,function(x) x$predict)
pcr_cpreds=lapply(pcr_cpreds,function(x) select(x,c("pcr","cpred")))

# Get AUC, sens, and spec for each iteration and calculate mean AUC and standard error across iterations
pcr_auc = lapply(pcr_cpreds, function(x) auc(x$pcr, x$cpred))
...
### COMPETENCE MODEL: Get model performance measures assuming 0.5 threshold ###

# Get predicted probabilities from competence model for each iteration
comp_cpreds=lapply(comp_brts,function(x) x$predict)
comp_cpreds=lapply(comp_cpreds,function(x) select(x,c("competence","cpred")))

# Get AUC, sens, and spec for each iteration
comp_auc = lapply(comp_cpreds, function(x) auc(x$comp, x$cpred))
...
# Get auc values for infection model
adata_pcr=data.frame(y=c(sapply(pcr_auc, function(x) x)))
adata_pcr$seed = 1:nrow(adata_pcr)
adata_pcr$response = "infection"

# Get auc values for competence model
adata_comp=data.frame(y=c(sapply(comp_auc, function(x) x)))
adata_comp$seed = 1:nrow(adata_comp)
adata_comp$response = "competence"
adata_auc_host=rbind(adata_pcr, adata_comp)

# Transform 'response' to factor
adata_auc_host$response=factor(adata_auc_host$response,levels=c('infection','competence'))
 
# Get sensitivity values for infection model
adata_pcr=data.frame(y=c(sapply(pcr_sens, function(x) x)))
adata_pcr$seed = 1:nrow(adata_pcr)
adata_pcr$response = "infection"

# Get specificity values for competence model
adata_comp=data.frame(y=c(sapply(comp_sens, function(x) x)))
adata_comp$seed = 1:nrow(adata_comp)
adata_comp$response = "competence"
adata_sens_host=rbind(adata_pcr, adata_comp)

# Transform 'response' to factor
adata_sens_host$response=factor(adata_sens_host$response,levels=c('infection','competence'))
 
# Get specificity values for infection model
...
adata_comp$seed = 1:nrow(adata_comp)
adata_comp$response = "competence"
adata_spec_host=rbind(adata_pcr, adata_comp)

# Transform 'response' to factor
adata_spec_host$response=factor(adata_spec_host$response,levels=c('infection','competence'))

# Clean
rm(list=setdiff(ls(), c("brts1", "brts2", 
                        "adata_auc_link", "adata_sens_link", "adata_spec_link",
                        "pcr_brts", "comp_brts", 
                        "adata_auc_host", "adata_sens_host", "adata_spec_host")))
```
...
# Combine link prediction with host trait model perfs
adata_auc <- rbind(adata_auc_link, adata_auc_host)
adata_sens <- rbind(adata_sens_link, adata_sens_host)
adata_spec <- rbind(adata_spec_link, adata_spec_host)

# Check factor levels of the response variable
levels(adata_auc$response)
levels(adata_sens$response)
levels(adata_spec$response)

# Clean environment
rm(adata_auc_link, adata_auc_host, adata_sens_link, adata_sens_host, adata_spec_link, adata_spec_host)
...
# Boxplot of AUC
png("Output/boxplot_model_auc_nature.png",width=6,height=6,units="in",res=600)
set.seed(1)
ggplot(df_auc)+
  geom_boxplot(aes(x=x,y=y,group=x),width=0.7,alpha=0.25,outlier.alpha=0)+
  geom_point(aes(x=xj,y=y),size=1.5,alpha=0.5)+
  scale_x_continuous(breaks=c(1,2,3,4),
  labels=c("Host-virus links","Host-virus links \n excluding VACV", "RT-PCR \n positivity", "Virus isolation"),
  limits=c(0.5,4.5))+
  theme_bw()+
  labs(x="Response variable",
       y="Area under the curve (AUC)")+
...
png("Output/boxplot_model_senspec_nature.png",width=6,height=6,units="in",res=300)
set.seed(1)
ggplot(sdata) +
  geom_boxplot(aes(x=x,y=y,group=x),width=0.7,alpha=0.25,outlier.alpha=0)+
  geom_point(aes(x=xj,y=y),size=1.5,alpha=0.5)+
...
Figures: */Nature/plot_pca_scoresbyspecies_v1.pdf*, */Nature/plot_pca_scoresbyspecies_v2.pdf*
...
# Combine plots and save
  library(cowplot)
  pdf("Output/Nature/plot_pca_scoresbyspecies.pdf", width=8,height=8)
...
# Read in function data (compiled by H.)
    ...
    # For PC3 positive and negative loadings: read in data, rename columns, and merge
    pc3_fnx_pos <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC3")[,1:3]
    pc3_fnx_neg <- read_xlsx("PCA_Gene_contributions-classification.xlsx", sheet="PC3")[,7:8]
    pc3_fnx_neg$role <- NA
...
## Reclassify gene functions to create fewer groups for plotting
    
# Get function names
unique(genes_df$role)

# Create new group 'Antagonizing host innate immunity'
genes_df$role_simple <- genes_df$role
genes_df$role_simple <- ifelse(genes_df$role_simple=="Cell death/cell cycle regulation"|
                        genes_df$role_simple=="Chemokine/cytokine regulation"|
                        genes_df$role_simple=="Pathogen recognition"|
                        genes_df$role_simple=="Ankyrin"|
                        genes_df$role_simple=="Kelch-like protein",
                        "Antagonizing host innate immunity",genes_df$role_simple)

# Create new group 'Replication/transcription'
genes_df$role_simple <- ifelse(genes_df$role_simple=="DNA replication"|
                        genes_df$role_simple=="RNA/transcription/translation",
                        "Viral replication/transcription",genes_df$role_simple)
...
# Create loading plots by gene function
  ...
  ## Create color-blind friendly palette based on number of roles excluding 'Not classified'
    ...
    # Explore color palettes using <carto_pal>: add light-gray <#D3D3D3> for not classified and dark-gray <#6F7378> for unknown 
    demoplot(carto_pal(6,"Bold"), type="bar")
    print(carto_pal(6,"Bold"))
    demoplot(c("#7F3C8D", "#11A579", "#D3D3D3", "#3969AC", "#6F7378", "#F2B701", "#E73F74"), type="bar")
    
    # Use Nature-ish color palette: https://nanx.me/ggsci/reference/pal_npg.html
    demoplot(c("#E64B35FF", "#F2B701", "#D3D3D3", "#767676", "#00A087FF", "#87CFEE", "#F39B7FFF"), type="bar")
    cbPalette$colors <- c("#E64B35FF", "#F2B701", "#D3D3D3", "#767676", "#00A087FF", "#87CFEE", "#F39B7FFF")
  ...
  ## PC1 and PC2
    ...
    # are all 7 gene functions present for these dimensions? 
    sort(unique(genes_df_pc12$role_simple))

    # get hex codes from cbPalette specific to plotting PC1 and PC2
    colors <- cbPalette[cbPalette$role!="Not classified",c("colors")]
  ...
  ## PC3 and PC4
    ...
    # are all 7 gene functios present for these dimensions? 
    sort(unique(genes_df_pc34$role_simple))
    
    # get hex codes from cbPalette specific to plotting PC3 and PC4
    colors <- cbPalette[,c("colors")]
  ...
  ## PC5 and PC6
    ...
    # get hex codes from cbPalette specific to plotting PC5 and PC6
    colors <- cbPalette[,c("colors")]
  ...
  ## PC7 and PC8
    ...
    # get hex codes from cbPalette specific to plotting PC7 and PC8
    colors <- cbPalette[,c("colors")]
  ...
  ## PC9 and PC10
    ...
    # are all 7 gene functios present for these dimensions? 
    sort(unique(genes_df_pc910$role_simple))
  ...
  legend <- cowplot::get_legend(fviz_pca_var(pca, axes = c(9, 10),
               ...
               repel = TRUE, labelsize=1, label="none",
               select.var=list(name=pc910_gene)) +
               scale_color_manual(name = "Gene predicted function", labels = c("Antagonizing host adaptive immunity", "Antagonizing host innate immunity", "Viral replication/transcription", "Virus entry", "Virus structure", "Unknown function", "Not classified"),
                    values= c("#E64B35FF", "#F2B701", "#00A087FF", "#87CFEE", "#F39B7FFF", "#767676", "#D3D3D3")))
             
  ## Combine biplots and save
  library(cowplot)
  pdf("Output/Nature/plot_pca_loadingsbyrole.pdf", width=10,height=8)
...
### ALTERNATIVE PLOTS ###
  ...
  ## Create loading plots for the most influential genes for each PC
...
9. Nature Micro: Proportions plots
============================================
...
# Save dataset of the most influential genes, their loadings, and their functional roles
  ...
  ## Export
  library(openxlsx)
  write.xlsx(list_df, file='Output/Nature/table_rankedgenes.xlsx')

# Create proportions/dot plots for gene functional roles

  ## To each df in list_df, add column containing df's name
  list_df <- Map(cbind, list_df, dimension = names(list_df))
 
  ## Unlist and merge all rows of data
  gene_roles <- rbind(list_df[[1]], list_df[[2]], list_df[[3]], list_df[[4]], list_df[[5]], list_df[[6]], list_df[[7]], list_df[[8]])

    # Rename dimensions to combine positive and negative loadings
    gene_roles$dimension <- ifelse(gene_roles$dimension=="pc1_pos"|gene_roles$dimension=="pc1_neg", "PC1",gene_roles$dimension)
    gene_roles$dimension <- ifelse(gene_roles$dimension=="pc3_pos"|gene_roles$dimension=="pc3_neg", "PC3",gene_roles$dimension)
    gene_roles$dimension <- ifelse(gene_roles$dimension=="pc4_pos"|gene_roles$dimension=="pc4_neg", "PC4",gene_roles$dimension)
    gene_roles$dimension <- ifelse(gene_roles$dimension=="pc9_pos"|gene_roles$dimension=="pc9_neg", "PC9",gene_roles$dimension)

# Create color palette
    
    # Create empty df 
    # cbPalette <- sort(unique(gene_roles$role_simple))
    # cbPalette <- as.data.frame(cbPalette[! cbPalette %in% c('Not classified')])
    cbPalette <- as.data.frame(sort(unique(gene_roles$role_simple)))
    colnames(cbPalette) <- c("role_simple")

    # Color palette V2 for 6 functional groups: Nature-ish (https://nanx.me/ggsci/reference/pal_npg.html)
    library(colorspace)
    demoplot(c("#E64B35FF", "#F2B701", "#767676", "#00A087FF", "#87CFEE", "#F39B7FFF"), type="bar")
    # cbPalette$colors <- c("#E64B35FF", "#F2B701", "#767676", "#00A087FF", "#87CFEE", "#F39B7FFF")
    cbPalette$colors <- c("#E64B35FF", "#F2B701", "#5A5A5A", "#767676", "#00A087FF", "#87CFEE", "#F39B7FFF")

# For reference, determine number of influential genes per PC and get proportions (and counts) by function
length(which(gene_roles$dimension=="PC1"))
length(which(gene_roles$dimension=="PC3"))
length(which(gene_roles$dimension=="PC4"))
length(which(gene_roles$dimension=="PC9"))
props <- gene_roles %>% dplyr::group_by(dimension, role_simple) %>% summarise(n = n()) %>% mutate(freq = n/sum(n))

# Plot proportions: size and group by x/y variables to have rows sum to 1
p <- ggplot(gene_roles, aes(x = role_simple, y = dimension))
prop_plot <- p + geom_count(aes(size = after_stat(prop), group = dimension, colour = role_simple)) +
    scale_size_area(max_size = 20) +
    scale_colour_manual(values = cbPalette$colors) +
...
## Save plot
pdf("Output/Nature/plot_pca_proportionsbyrole.pdf", width=8,height=5)
prop_plot 
dev.off()  

#### Plot a version dropping out "Not classified" genes ####

# Plot proportions: size and group by x/y variables to have rows sum to 1
which(gene_roles$role_simple=="Not classified")
gene_roles_noNA <- gene_roles[gene_roles$role_simple != "Not classified",]
cbPalette_noNA <- cbPalette[cbPalette$role_simple != "Not classified",]

p <- ggplot(gene_roles_noNA, aes(x = role_simple, y = dimension))
prop_plot <- p + geom_count(aes(size = after_stat(prop), group = dimension, colour = role_simple)) +
    scale_size_area(max_size = 20) +
    scale_colour_manual(values = cbPalette_noNA$colors) +
    scale_x_discrete(position = "top") +
    theme(panel.background = element_blank(),
          panel.grid.major.x = element_blank(), 
          panel.grid.major.y = element_line(size=.5, color="gray" ),
...
## Save plot
pdf("Output/Nature/plot_pca_proportionsbyrole_noNA.pdf", width=8,height=5)
prop_plot 
dev.off()  
```
