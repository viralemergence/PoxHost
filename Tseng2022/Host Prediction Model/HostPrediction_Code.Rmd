...
The following code reproduces the analyses from <...>, pertaining to the host prediction model. 
...
To run the following script, two files are required in your home directory:
(1) Data_raw.RData, the raw data file; and
...
#(6) Clean environment
rm(virion, opvgenes, vertlife, dryad, combine)
...
library(ggpubr)
png("Output/Figure 1.png",width=6,height=6,units="in",res=300)
...
png("Output/Figure S3.png",width=4,height=5,units="in",res=300)
set.seed(1)
ggplot(sdata)+
...
write.csv(ts5,"Output/Table S5.csv")
...
png("Output/Figure 2.png",width=7,height=3.5,units="in",res=300)
...
png("Output/Figure S4.png",width=4,height=10,units="in",res=300)
...
#(21) Write file
write.csv(preds,"Output/PoxHost predictions.csv")
...
png("Output/Figure 3.png",width=6.5,height=4,units="in",res=300)
...
#(13) Write
write.csv(predpfs,"Output/Table S6.csv")
...
#(13) Revise Figure 3
png("Output/Figure 3.png",width=7,height=7.25,units="in",res=300)
