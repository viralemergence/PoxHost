...
#(2) trim unnecessary data columns
colnames(pfdata)
data <- pfdata[c(2:13)]

#(3) to remove duplicate genera, aggregate pcr and competence by genus, taking the max value
agg_pcr <- aggregate(pcr~gen, data=data, max)
agg_competence <- aggregate(competence~gen, data=data, max)

#(4) remove duplicate rows/genera, and merge pcr and competence data back in
data$pcr=NULL
...
#(6) create variables label and Species (required in later functions)f
data$label <- data$treename
data$Species <- data$treename  

#(7) merge phylogeny with dataset ensuring consistent structure and ordering (caper::comparative.data)
