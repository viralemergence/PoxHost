...
## extract studies of orthopoxvirus detection via PCR or isolation
poxdata <- virion %>%
...
## exclude (1) host or virus is NA and (2) variola virus
poxdata <- poxdata[!is.na(poxdata$Host),]
poxdata <- poxdata[!is.na(poxdata$Virus),]
poxdata <- poxdata[!(poxdata$Virus=="variola virus"),]

## subset monkeypox interactions 
mpxvdata <- poxdata %>% filter(Virus=="monkeypox virus")
write.csv(mpxvdata, "/Users/katietseng/Downloads/temp.csv")
