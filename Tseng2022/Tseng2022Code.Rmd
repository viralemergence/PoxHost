...
poxdata_all <- merge(combinations,interactions,by=c("gen","virus"),all.x=TRUE)

#(3) merge host-related data
poxdata_all <- merge(poxdata_all,poxdata_host,by=c("gen"),all.x=TRUE)

#(6) create binary variable for sampled host-OPV pairs
poxdata_all$sampled=ifelse(is.na(poxdata_all$pcr) & is.na(poxdata_all$competence),0,1)

#(7) reclassify NAs as pseudo-absences for viral detection
poxdata_all$pcr=ifelse(is.na(poxdata_all$pcr),0,poxdata_all$pcr)
poxdata_all$competence=ifelse(is.na(poxdata_all$competence),0,poxdata_all$competence)
poxdata_all$studies=ifelse(is.na(poxdata_all$studies),0,poxdata_all$studies)
...
poxdata_all <- merge(poxdata_all,viralTraits,by=c('virus'),all.x=TRUE)
...
poxdata$compcr <- ifelse(poxdata$pcr==1|poxdata$competence==1,1,0)
